...
// truncated toward zero. time.Time values are stored as Unix nanoseconds, the
// same unit that *big.Int → time.Time uses, making the round-trip lossless for
// times that fit in int64.
//
// Options:
...
func toBigInt(v any, ops Ops) (any, error) {
	...
	case float64:
		...
		return i, nil
	case time.Time:
		return big.NewInt(val.UnixNano()), nil
	default:
