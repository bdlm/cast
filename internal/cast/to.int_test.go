...
// TestABSWithFloat64 covers the math.Floor(-val) branch in toInt for float64 inputs.
func TestABSWithFloat64(t *testing.T) {
	t.Run("negative float64 → uint with ABS=true", func(t *testing.T) {
		result, err := cast.ToE[uint](float64(-3.9), cast.Op{cast.ABS, true})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if result != uint(3) {
			t.Errorf("expected 3 (floor of 3.9), got %v", result)
		}
	})
	t.Run("negative float64 → uint without ABS errors", func(t *testing.T) {
		_, err := cast.ToE[uint](float64(-3.9))
		if err == nil {
			t.Error("expected error for negative float64 → uint without ABS")
		}
	})
	t.Run("negative float64 → uint32 with ABS=true", func(t *testing.T) {
		result, err := cast.ToE[uint32](float64(-12.7), cast.Op{cast.ABS, true})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if result != uint32(12) {
			t.Errorf("expected 12 (floor of 12.7), got %v", result)
		}
	})
}

// TestABSWithNegativeFloatString covers the strToInt path: a string that parses
// to a negative float64 with ABS=true uses the absolute value before flooring.
func TestABSWithNegativeFloatString(t *testing.T) {
	t.Run(`"-3.7" → uint with ABS=true → 3`, func(t *testing.T) {
		result, err := cast.ToE[uint]("-3.7", cast.Op{cast.ABS, true})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if result != uint(3) {
	...
	t.Run(`"-100" → uint with ABS=true → 100`, func(t *testing.T) {
		result, err := cast.ToE[uint]("-100", cast.Op{cast.ABS, true})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if result != uint(100) {
	...
	t.Run(`"-3.7" → uint without ABS errors`, func(t *testing.T) {
		_, err := cast.ToE[uint]("-3.7")
		if err == nil {
			t.Error("expected error for negative float string → uint without ABS")
		}
	})
}
...
// TestMapToIntErrors verifies that map sources always error for int* and uint*
// targets (documented as ✗ in the conversion table).
func TestMapToIntErrors(t *testing.T) {
	src := map[string]int{"a": 1}
	for _, tc := range []struct {
		name string
		run  func() error
	}{
		{"map → int", func() error { _, err := cast.ToE[int](src); return err }},
		{"map → int8", func() error { _, err := cast.ToE[int8](src); return err }},
		{"map → int16", func() error { _, err := cast.ToE[int16](src); return err }},
		{"map → int32", func() error { _, err := cast.ToE[int32](src); return err }},
		{"map → int64", func() error { _, err := cast.ToE[int64](src); return err }},
		{"map → uint", func() error { _, err := cast.ToE[uint](src); return err }},
		{"map → uint8", func() error { _, err := cast.ToE[uint8](src); return err }},
		{"map → uint16", func() error { _, err := cast.ToE[uint16](src); return err }},
		{"map → uint32", func() error { _, err := cast.ToE[uint32](src); return err }},
		{"map → uint64", func() error { _, err := cast.ToE[uint64](src); return err }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			err := tc.run()
			if err == nil {
...
// TestStructToUintError verifies that struct sources error for uint* targets
// (documented as ✗; struct → int* is already covered by TestToIntStructSource).
func TestStructToUintError(t *testing.T) {
	type point struct{ X int }
	_, err := cast.ToE[uint](point{X: 1})
	if err == nil {
		t.Error("expected error for struct → uint, got nil")
	}
	if !errors.Is(err, cast.ErrorUnableToCast) {
		t.Errorf("expected cast.ErrorUnableToCast, got %v", err)
	}
