...
// TestMapFromAdditionalScalarErrors verifies that bool, uint, float, and complex
// scalar sources always error for map targets (documented as ✗). Int is already
// covered by TestMapFromScalarSourceError.
func TestMapFromAdditionalScalarErrors(t *testing.T) {
	for _, tc := range []struct {
		...
		{"bool → map", true},
		{"uint → map", uint(5)},
		{"float64 → map", float64(1.5)},
		{"complex128 → map", complex128(1 + 2i)},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, err := cast.ToE[map[string]int](tc.src)
			if err == nil {
...
// TestMapFromByteSlice verifies that []byte ([]uint8) and []rune ([]int32)
// sources work for map targets — documented as ~ᵐ (elements cast individually).
func TestMapFromByteSlice(t *testing.T) {
	t.Run("[]byte → map[int]uint8 (elements individually)", func(t *testing.T) {
		result, err := cast.ToE[map[int]uint8]([]byte{10, 20, 30})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("[]rune → map[int]int32 (elements individually)", func(t *testing.T) {
		result, err := cast.ToE[map[int]int32]([]rune{'a', 'b', 'c'})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
