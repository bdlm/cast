...
// *big.Int and big.Int values are treated as Unix nanoseconds (int64 range only).
...
//
// When format is "" (no FORMAT option): all 19 entries in timeFormats are tried
// in order. After the loop, time.Parse("", str) is attempted unconditionally.
// This means the empty string "" is a valid source: time.Parse("", "") succeeds
// and returns the zero time.Time{}.
//
// When format is non-empty (FORMAT option set): the 19-format loop is skipped
// entirely and only time.Parse(format, str) is tried.
