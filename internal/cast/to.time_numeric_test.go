...
func TestTimeToBigConversions(t *testing.T) {
	...
	t.Run("*big.Int: 1s → 1000000000ns", func(t *testing.T) {
		result, err := cast.ToE[*big.Int](oneSecond)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if result == nil || result.Int64() != int64(time.Second) {
			t.Errorf("expected %d, got %v", int64(time.Second), result)
		}
	})
	t.Run("*big.Int: pre-epoch → -1000000000ns", func(t *testing.T) {
